...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{../../specification/hashmap.pdf}
    \caption{The hashmap specification}
    \label{fig:hashmap-spec}
\end{figure}
The base hashmap implementation uses multisets (called "bags" in TLA+) to maintain a set of keys. This is in order to make it possible to have erronous key duplication, because otherwise the semantics of sets would do the job for us.
\\\\
The map itself is a function from keys to values. Formally it is a non-surjective, non-injective function from the domain of possible keys to the domain of possible values. Initially the value of the function is the constant NULL for all keys.
\\\\
Insert and remove are the two possible operations. Insert picks a key from possible keys, a value from possible values, and attempts to add the key-value pair to the map. Remove only picks a key, and then removes both the key and the value from the map. The Next formula is the disjunction of these operations meaning that each step can be either an insert or a remove.
\\\\
In this initial version of the map we test for type correctness, key uniqueness and the presence of values for keys in the map.
